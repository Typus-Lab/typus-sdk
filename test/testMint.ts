...
(async () => {
  ...
  let mintTx = await getMintTx(gasBudget, TOKEN_PACKAGE, tokenRegistry, moduleName, mintAmount);
  let moveCallTxn = await signer.signAndExecuteTransactionBlock(mintTx);
...
async function checkData(moveCallTxn, tokenRegistry: string) {
  try {
    ...
    let txn = await provider.getTransactionWithEffects(digest);
