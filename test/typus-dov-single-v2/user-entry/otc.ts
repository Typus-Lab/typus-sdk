...
(async () => {
    ...
    otc(config, transaction, {
        typeArguments: [vault.info.depositToken, vault.info.bidToken],
        index,
        coins: (await provider.getCoins({ owner: user, coinType: vault.info.bidToken })).data.map((coin) => coin.coinObjectId),
        amount,
    });
