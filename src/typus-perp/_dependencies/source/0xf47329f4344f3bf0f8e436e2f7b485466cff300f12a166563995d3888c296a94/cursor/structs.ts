...
export class Cursor<T extends TypeArgument> implements StructClass {
    ...
    private constructor(typeArgs: [ToTypeStr<T>], fields: CursorFields<T>) {
        this.$fullTypeName = composeSuiType(Cursor.$typeName, ...typeArgs) as `${typeof PKG_V1}::cursor::Cursor<${ToTypeStr<T>}>`;
    ...
    static fromSuiObjectData<T extends Reified<TypeArgument, any>>(typeArg: T, data: SuiObjectData): Cursor<ToTypeArgument<T>> {
        if (data.bcs) {
