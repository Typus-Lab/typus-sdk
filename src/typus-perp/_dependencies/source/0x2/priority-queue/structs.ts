...
export class Entry<T extends TypeArgument> implements StructClass {
    ...
    static readonly $typeName = `${PKG_V26}::priority_queue::Entry`;
    static readonly $numTypeParams = 1;
    static readonly $isPhantom = [false] as const;

    readonly $typeName = Entry.$typeName;
    readonly $fullTypeName: `${typeof PKG_V26}::priority_queue::Entry<${ToTypeStr<T>}>`;
    ...
    private constructor(typeArgs: [ToTypeStr<T>], fields: EntryFields<T>) {
        this.$fullTypeName = composeSuiType(Entry.$typeName, ...typeArgs) as `${typeof PKG_V26}::priority_queue::Entry<${ToTypeStr<T>}>`;
    ...
    static reified<T extends Reified<TypeArgument, any>>(T: T): EntryReified<ToTypeArgument<T>> {
        return {
            ...
            fullTypeName: composeSuiType(
                Entry.$typeName,
                ...[extractType(T)]
            ) as `${typeof PKG_V26}::priority_queue::Entry<${ToTypeStr<ToTypeArgument<T>>}>`,
...
export class PriorityQueue<T extends TypeArgument> implements StructClass {
    ...
    static readonly $typeName = `${PKG_V26}::priority_queue::PriorityQueue`;
    static readonly $numTypeParams = 1;
    static readonly $isPhantom = [false] as const;

    readonly $typeName = PriorityQueue.$typeName;
    readonly $fullTypeName: `${typeof PKG_V26}::priority_queue::PriorityQueue<${ToTypeStr<T>}>`;
    readonly $typeArgs: [ToTypeStr<T>];
    ...
    private constructor(typeArgs: [ToTypeStr<T>], fields: PriorityQueueFields<T>) {
        this.$fullTypeName = composeSuiType(
            ...
            ...typeArgs
        ) as `${typeof PKG_V26}::priority_queue::PriorityQueue<${ToTypeStr<T>}>`;
        this.$typeArgs = typeArgs;
    ...
    static reified<T extends Reified<TypeArgument, any>>(T: T): PriorityQueueReified<ToTypeArgument<T>> {
        return {
            ...
            fullTypeName: composeSuiType(
                PriorityQueue.$typeName,
                ...[extractType(T)]
            ) as `${typeof PKG_V26}::priority_queue::PriorityQueue<${ToTypeStr<ToTypeArgument<T>>}>`,
            typeArgs: [extractType(T)] as [ToTypeStr<ToTypeArgument<T>>],
    ...
    static fromJSON<T extends Reified<TypeArgument, any>>(typeArg: T, json: Record<string, any>): PriorityQueue<ToTypeArgument<T>> {
        ...
        assertReifiedTypeArgsMatch(composeSuiType(PriorityQueue.$typeName, extractType(typeArg)), json.$typeArgs, [typeArg]);
