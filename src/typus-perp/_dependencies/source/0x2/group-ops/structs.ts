...
export function isElement(type: string): boolean {
    ...
    return type.startsWith(`${PKG_V26}::group_ops::Element` + "<");
}
...
export class Element<T extends PhantomTypeArgument> implements StructClass {
    ...
    static readonly $typeName = `${PKG_V26}::group_ops::Element`;
    static readonly $numTypeParams = 1;
    static readonly $isPhantom = [true] as const;

    readonly $typeName = Element.$typeName;
    readonly $fullTypeName: `${typeof PKG_V26}::group_ops::Element<${PhantomToTypeStr<T>}>`;
    ...
    private constructor(typeArgs: [PhantomToTypeStr<T>], fields: ElementFields<T>) {
        this.$fullTypeName = composeSuiType(
            Element.$typeName,
            ...typeArgs
        ) as `${typeof PKG_V26}::group_ops::Element<${PhantomToTypeStr<T>}>`;
    ...
    static reified<T extends PhantomReified<PhantomTypeArgument>>(T: T): ElementReified<ToPhantomTypeArgument<T>> {
        return {
            ...
            fullTypeName: composeSuiType(
                Element.$typeName,
                ...[extractType(T)]
            ) as `${typeof PKG_V26}::group_ops::Element<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`,
