...
export class Receiving<T extends PhantomTypeArgument> implements StructClass {
    ...
    static readonly $typeName = `${PKG_V26}::transfer::Receiving`;
    static readonly $numTypeParams = 1;
    static readonly $isPhantom = [true] as const;

    readonly $typeName = Receiving.$typeName;
    readonly $fullTypeName: `${typeof PKG_V26}::transfer::Receiving<${PhantomToTypeStr<T>}>`;
    ...
    private constructor(typeArgs: [PhantomToTypeStr<T>], fields: ReceivingFields<T>) {
        this.$fullTypeName = composeSuiType(
            Receiving.$typeName,
            ...typeArgs
        ) as `${typeof PKG_V26}::transfer::Receiving<${PhantomToTypeStr<T>}>`;
    ...
    static reified<T extends PhantomReified<PhantomTypeArgument>>(T: T): ReceivingReified<ToPhantomTypeArgument<T>> {
        return {
            ...
            fullTypeName: composeSuiType(
                Receiving.$typeName,
                ...[extractType(T)]
            ) as `${typeof PKG_V26}::transfer::Receiving<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`,
