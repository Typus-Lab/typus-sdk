...
export class Balance<T extends PhantomTypeArgument> implements StructClass {
    ...
    static readonly $typeName = `${PKG_V26}::balance::Balance`;
    ...
    private constructor(typeArgs: [PhantomToTypeStr<T>], fields: BalanceFields<T>) {
        this.$fullTypeName = composeSuiType(
            Balance.$typeName,
            ...typeArgs
        ) as `${typeof PKG_V26}::balance::Balance<${PhantomToTypeStr<T>}>`;
    ...
    static reified<T extends PhantomReified<PhantomTypeArgument>>(T: T): BalanceReified<ToPhantomTypeArgument<T>> {
        return {
            ...
            fullTypeName: composeSuiType(
                Balance.$typeName,
                ...[extractType(T)]
            ) as `${typeof PKG_V26}::balance::Balance<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`,
...
export class Supply<T extends PhantomTypeArgument> implements StructClass {
    ...
    static readonly $typeName = `${PKG_V26}::balance::Supply`;
    static readonly $numTypeParams = 1;
    static readonly $isPhantom = [true] as const;
    ...
    private constructor(typeArgs: [PhantomToTypeStr<T>], fields: SupplyFields<T>) {
        this.$fullTypeName = composeSuiType(Supply.$typeName, ...typeArgs) as `${typeof PKG_V26}::balance::Supply<${PhantomToTypeStr<T>}>`;
        this.$typeArgs = typeArgs;
    ...
    static reified<T extends PhantomReified<PhantomTypeArgument>>(T: T): SupplyReified<ToPhantomTypeArgument<T>> {
        return {
            ...
            fullTypeName: composeSuiType(
                Supply.$typeName,
                ...[extractType(T)]
            ) as `${typeof PKG_V26}::balance::Supply<${PhantomToTypeStr<ToPhantomTypeArgument<T>>}>`,
            typeArgs: [extractType(T)] as [PhantomToTypeStr<ToPhantomTypeArgument<T>>],
