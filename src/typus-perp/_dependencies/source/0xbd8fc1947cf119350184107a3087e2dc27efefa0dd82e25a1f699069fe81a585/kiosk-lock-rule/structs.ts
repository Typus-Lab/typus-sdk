...
export function isConfig(type: string): boolean {
    ...
    return type === `${PKG_V1}::kiosk_lock_rule::Config`;
}
...
export class Config implements StructClass {
    ...
    static readonly $typeName = `${PKG_V1}::kiosk_lock_rule::Config`;
    ...
    private constructor(typeArgs: [], fields: ConfigFields) {
        this.$fullTypeName = composeSuiType(Config.$typeName, ...typeArgs) as `${typeof PKG_V1}::kiosk_lock_rule::Config`;
    ...
    static reified(): ConfigReified {
        return {
            ...
            fullTypeName: composeSuiType(Config.$typeName, ...[]) as `${typeof PKG_V1}::kiosk_lock_rule::Config`,
            typeArgs: [] as [],
...
export function isRule(type: string): boolean {
    type = compressSuiType(type);
    return type === `${PKG_V1}::kiosk_lock_rule::Rule`;
}
...
export class Rule implements StructClass {
    ...
    static readonly $typeName = `${PKG_V1}::kiosk_lock_rule::Rule`;
    static readonly $numTypeParams = 0;
    static readonly $isPhantom = [] as const;
    ...
    private constructor(typeArgs: [], fields: RuleFields) {
        this.$fullTypeName = composeSuiType(Rule.$typeName, ...typeArgs) as `${typeof PKG_V1}::kiosk_lock_rule::Rule`;
        this.$typeArgs = typeArgs;
    ...
    static reified(): RuleReified {
        return {
            ...
            fullTypeName: composeSuiType(Rule.$typeName, ...[]) as `${typeof PKG_V1}::kiosk_lock_rule::Rule`,
            typeArgs: [] as [],
