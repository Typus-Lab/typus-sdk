...
export class PriceFeedUpdateEvent implements StructClass {
    ...
    private constructor(typeArgs: [], fields: PriceFeedUpdateEventFields) {
        this.$fullTypeName = composeSuiType(PriceFeedUpdateEvent.$typeName, ...typeArgs) as `${typeof PKG_V1}::event::PriceFeedUpdateEvent`;
    ...
    static reified(): PriceFeedUpdateEventReified {
        return {
            ...
            fullTypeName: composeSuiType(PriceFeedUpdateEvent.$typeName, ...[]) as `${typeof PKG_V1}::event::PriceFeedUpdateEvent`,
            typeArgs: [] as [],
            isPhantom: PriceFeedUpdateEvent.$isPhantom,
            reifiedTypeArgs: [],
            fromFields: (fields: Record<string, any>) => PriceFeedUpdateEvent.fromFields(fields),
            fromFieldsWithTypes: (item: FieldsWithTypes) => PriceFeedUpdateEvent.fromFieldsWithTypes(item),
            ...
            fromSuiParsedData: (content: SuiParsedData) => PriceFeedUpdateEvent.fromSuiParsedData(content),
            fromSuiObjectData: (content: SuiObjectData) => PriceFeedUpdateEvent.fromSuiObjectData(content),
    ...
    static fromSuiParsedData(content: SuiParsedData): PriceFeedUpdateEvent {
        ...
        if (!isPriceFeedUpdateEvent(content.type)) {
            throw new Error(`object at ${(content.fields as any).id} is not a PriceFeedUpdateEvent object`);
        }
...
export class PythInitializationEvent implements StructClass {
    ...
    static reified(): PythInitializationEventReified {
        return {
            ...
            fullTypeName: composeSuiType(PythInitializationEvent.$typeName, ...[]) as `${typeof PKG_V1}::event::PythInitializationEvent`,
            typeArgs: [] as [],
            isPhantom: PythInitializationEvent.$isPhantom,
            reifiedTypeArgs: [],
            fromFields: (fields: Record<string, any>) => PythInitializationEvent.fromFields(fields),
            fromFieldsWithTypes: (item: FieldsWithTypes) => PythInitializationEvent.fromFieldsWithTypes(item),
            ...
            fromSuiParsedData: (content: SuiParsedData) => PythInitializationEvent.fromSuiParsedData(content),
            fromSuiObjectData: (content: SuiObjectData) => PythInitializationEvent.fromSuiObjectData(content),
    ...
    static fromSuiParsedData(content: SuiParsedData): PythInitializationEvent {
        ...
        if (!isPythInitializationEvent(content.type)) {
            throw new Error(`object at ${(content.fields as any).id} is not a PythInitializationEvent object`);
        }
    ...
    static async fetch(client: SuiClient, id: string): Promise<PythInitializationEvent> {
        const res = await client.getObject({ id, options: { showBcs: true } });
        if (res.error) {
            throw new Error(`error fetching PythInitializationEvent object at id ${id}: ${res.error.code}`);
        }
