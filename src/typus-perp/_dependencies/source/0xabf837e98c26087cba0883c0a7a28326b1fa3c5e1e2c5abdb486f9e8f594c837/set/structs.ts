...
export class Set<A extends TypeArgument> implements StructClass {
    ...
    private constructor(typeArgs: [ToTypeStr<A>], fields: SetFields<A>) {
        this.$fullTypeName = composeSuiType(Set.$typeName, ...typeArgs) as `${typeof PKG_V1}::set::Set<${ToTypeStr<A>}>`;
    ...
    static fromSuiObjectData<A extends Reified<TypeArgument, any>>(typeArg: A, data: SuiObjectData): Set<ToTypeArgument<A>> {
        if (data.bcs) {
...
export class Unit implements StructClass {
    ...
    private constructor(typeArgs: [], fields: UnitFields) {
        this.$fullTypeName = composeSuiType(Unit.$typeName, ...typeArgs) as `${typeof PKG_V1}::set::Unit`;
        this.$typeArgs = typeArgs;
