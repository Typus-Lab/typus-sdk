...
export async function newGameTx(
    ...
    tx.moveCall({
        target: `${packageId}::combo_dice::new_game`,
        typeArguments,
        arguments: [tx.object(registry), tx.object(exp_registry), tx.pure(index), tx.makeMoveVec({ objects: coins.map((id) => tx.object(id)) }), tx.pure(amount)],
    });
...
export async function playGuessTx(
    ...
    tx.moveCall({
        target: `${packageId}::combo_dice::play_guess`,
        typeArguments: [],
        arguments: [
            tx.object(registry),
            tx.pure(index),
