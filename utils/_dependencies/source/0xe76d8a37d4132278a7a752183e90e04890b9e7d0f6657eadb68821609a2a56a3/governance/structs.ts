...
export function isWormholeVAAVerificationReceipt(type: string): boolean {
    ...
    return type === "0xe76d8a37d4132278a7a752183e90e04890b9e7d0f6657eadb68821609a2a56a3::governance::WormholeVAAVerificationReceipt";
}
...
export class WormholeVAAVerificationReceipt implements StructClass {
    static readonly $typeName =
        "0xe76d8a37d4132278a7a752183e90e04890b9e7d0f6657eadb68821609a2a56a3::governance::WormholeVAAVerificationReceipt";
    ...
    private constructor(typeArgs: [], fields: WormholeVAAVerificationReceiptFields) {
        this.$fullTypeName = composeSuiType(
            ...
            ...typeArgs
        ) as "0xe76d8a37d4132278a7a752183e90e04890b9e7d0f6657eadb68821609a2a56a3::governance::WormholeVAAVerificationReceipt";
    ...
    static reified(): WormholeVAAVerificationReceiptReified {
        return {
            ...
            typeArgs: [] as [],
            reifiedTypeArgs: [],
            fromFields: (fields: Record<string, any>) => WormholeVAAVerificationReceipt.fromFields(fields),
            fromFieldsWithTypes: (item: FieldsWithTypes) => WormholeVAAVerificationReceipt.fromFieldsWithTypes(item),
            ...
            fromSuiParsedData: (content: SuiParsedData) => WormholeVAAVerificationReceipt.fromSuiParsedData(content),
            fetch: async (client: SuiClient, id: string) => WormholeVAAVerificationReceipt.fetch(client, id),
    ...
    static fromSuiParsedData(content: SuiParsedData): WormholeVAAVerificationReceipt {
        ...
        if (!isWormholeVAAVerificationReceipt(content.type)) {
            throw new Error(`object at ${(content.fields as any).id} is not a WormholeVAAVerificationReceipt object`);
        }
    ...
    static async fetch(client: SuiClient, id: string): Promise<WormholeVAAVerificationReceipt> {
        ...
        if (res.error) {
            throw new Error(`error fetching WormholeVAAVerificationReceipt object at id ${id}: ${res.error.code}`);
        }
        if (res.data?.bcs?.dataType !== "moveObject" || !isWormholeVAAVerificationReceipt(res.data.bcs.type)) {
